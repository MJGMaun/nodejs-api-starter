...
const isAdmin = async (req, res, next) => {
    try {
        const token = req.header('Authorization').replace('Bearer ', '')
        const decoded = jwt.verify(token, process.env.JWT_SECRET) // get the payload {id: 'dasda3432423', iat: '542423423'}
        const user = await User.findOne({ _id: decoded._id, 'tokens.token': token, 'roles.role': 'admin' }) // 'tokens.token' → will find in array of tokens, foreach
        ...
        res.status(401).send({ error: 'Unable to proceed.' })
    }
